...
class MultiHeadAttention(nn.Module):
    def __init__(self, model_config):
        ...
        self.W_Q = nn.Linear(model_config['d_embedding'], 
                             model_config['n_heads'] * model_config['d_k'])
        self.W_K = nn.Linear(model_config['d_embedding'], 
                             model_config['n_heads'] * model_config['d_k'])
        self.W_V = nn.Linear(model_config['d_embedding'], 
                             model_config['n_heads'] * model_config['d_v'])
        self.Linear = nn.Linear(model_config['n_heads'] * model_config['d_v'], 
    ...
    def forward(self,Q,K,V,attn_mask, model_config):
        ...
        q_s = self.W_Q(Q).view(batch_size, -1, model_config['n_heads'], 
                               model_config['d_k']).transpose(1,2)
        k_s = self.W_K(K).view(batch_size, -1, model_config['n_heads'], 
                               model_config['d_k']).transpose(1,2)
        v_s = self.W_V(V).view(batch_size, -1, model_config['n_heads'], 
                               model_config['d_v']).transpose(1,2)
        attn_mask = attn_mask.unsqueeze(1).repeat(1, 
                                                  model_config['n_heads'], 1, 1)
        context, weights = ScaledDotProductionAttention()(q_s, k_s, v_s, 
                                                          attn_mask, 
                                                          model_config)
...
class PoswiseFeedForwardNet(nn.Module):
    def __init__(self, model_config, d_ff = 2048):
        ...
        self.conv2 = nn.Conv1d(in_channels = d_ff,
                               out_channels = model_config['d_embedding'],
                               kernel_size=1)
        self.layer_norm = nn.LayerNorm(model_config['d_embedding'])
...
class Transformer(nn.Module):
    def __init__(self, corpus, model_config):
        ...
        self.projection = nn.Linear(model_config['d_embedding'], 
                                    len(corpus.tgt_vocab),bias = False)
...
def greedy_decoder(model, enc_input, start_symbol, model_config, tgt_len):
    ...
    for i in range(0,tgt_len):
        ...
        dec_output,_,_ = model.decoder(dec_input, 
                                       enc_input, 
                                       enc_outputs, 
                                       model_config)
