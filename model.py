...
class ScaledDotProductionAttention(nn.Module):
    ...
    def forward(self, Q, K, V, mask, model_config):
...
class MultiHeadAttention(nn.Module):
    def __init__(self, model_config):
        ...
        self.W_Q = nn.Linear(model_config['d_embedding'], model_config['n_heads']*model_config['d_k'])
        self.W_K = nn.Linear(model_config['d_embedding'], model_config['n_heads']*model_config['d_k'])
        self.W_V = nn.Linear(model_config['d_embedding'], model_config['n_heads']*model_config['d_v'])
        self.Linear = nn.Linear(model_config['n_heads']*model_config['d_v'], model_config['d_embedding'])
        self.layer_norm = nn.LayerNorm(model_config['d_embedding'])

    def forward(self,Q,K,V,attn_mask, model_config):
        residual, batch_size = Q, Q.size(0)
        q_s = self.W_Q(Q).view(batch_size, -1, model_config['n_heads'], model_config['d_k']).transpose(1,2)
        k_s = self.W_K(K).view(batch_size, -1, model_config['n_heads'], model_config['d_k']).transpose(1,2)
        v_s = self.W_V(V).view(batch_size, -1, model_config['n_heads'], model_config['d_v']).transpose(1,2)
        attn_mask = attn_mask.unsqueeze(1).repeat(1, model_config['n_heads'], 1, 1)
        context, weights = ScaledDotProductionAttention()( q_s, k_s, v_s, attn_mask, model_config)
        context = context.transpose(1,2).contiguous().view(batch_size, -1, model_config['n_heads']*model_config['d_v'])
...
class PoswiseFeedForwardNet(nn.Module):
    def __init__(self, model_config, d_ff=2048):
        ...
        self.conv1 = nn.Conv1d(in_channels=model_config['d_embedding'], out_channels=d_ff, kernel_size=1)
        self.conv2 = nn.Conv1d(in_channels=d_ff, out_channels=model_config['d_embedding'], kernel_size=1)
        self.layer_norm = nn.LayerNorm(model_config['d_embedding'])
...
class Decoder(nn.Module):
    ...
    def forward(self,dec_inputs, enc_inputs, enc_outputs, model_config):
        ...
        for layer in self.layers:
            dec_outputs, dec_self_attn, dec_enc_attn = layer(dec_outputs, enc_outputs, dec_self_attn_mask, dec_enc_attn_mask, model_config)
...
class Transformer(nn.Module):
    def __init__(self, corpus, model_config):
        ...
        self.encoder = Encoder(corpus, model_config)
        self.decoder = Decoder(corpus, model_config)
        self.projection = nn.Linear(model_config['d_embedding'], len(corpus.tgt_vocab),bias = False)
        
    def forward(self, enc_inputs, dec_inputs, model_config):
        enc_outputs, enc_self_attns = self.encoder(enc_inputs, model_config)
        dec_outputs, dec_self_attns, dec_enc_attns = self.decoder(dec_inputs, enc_inputs, enc_outputs, model_config)
